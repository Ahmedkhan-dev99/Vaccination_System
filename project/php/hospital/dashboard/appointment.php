...
$sql = "SELECT * FROM appointments WHERE DATE(appointment_date) = CURDATE()";
$result = mysqli_query($conn, $sql);
$total = mysqli_num_rows($result);

$sql = "SELECT * FROM appointments WHERE DATE(appointment_date) = CURDATE() AND status = 'completed'";
$result = mysqli_query($conn, $sql);
$completed = mysqli_num_rows($result);

$sql = "SELECT * FROM appointments WHERE DATE(appointment_date) = CURDATE() AND status = 'pending'";
$result = mysqli_query($conn, $sql);
...
$output = '<div class="card border-0 slide-left" style="background: rgba(255, 255, 255, 0.95); box-shadow: 0 4px 15px rgba(0,0,0,0.1); border-radius: 15px;">
                    <div class="card-header" style="background: linear-gradient(45deg, #6a11cb, #2575fc); border-radius: 15px 15px 0 0;">
                        <h3 class="card-title mb-0 text-white"><i class="fas fa-calendar-check me-2"></i>Today\'s Appointments</h3>
                    </div>
                    <div class="card-body">
